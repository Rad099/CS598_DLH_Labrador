...
sys.path.append(os.path.join(repo_root, "labrador_original"))
...
# for compatibility with Colab environment
this_file = os.path.abspath(__file__)
scripts_dir = os.path.dirname(this_file)          # .../labrador/scripts/evaluations
repo_root = os.path.dirname(scripts_dir)         # .../labrador
...
from models.labrador_pooling import LabradorPoolingWrapper # extension: attn/gating pooling
...
print("[DEBUG] Example combo:", combos[0] if combos else "NONE")
            ...
            if config.get("train_config", {}).get("real_ensembling_samples", False):
                model = [
                    LabradorPoolingWrapper(
                        ...
                        == "true",
                        pooling_type=train_config_i.get("pooling_method", "attn"),
                    )
    ...
    if config.get("train_config", {}).get("real_ensembling_samples", False):
        model = [
            LabradorPoolingWrapper(
                ...
                == "true",
                pooling_type=best_hps.get("pooling_method", "attn"),
            )
            for _ in range(config["train_config"]["real_ensembling_samples"])
